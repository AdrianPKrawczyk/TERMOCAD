...
import type { TechnologyLabor } from '../../store/useAppStore';

interface LaborTableProps {
  laborEntries: TechnologyLabor[];
  onUpdate: (labor: TechnologyLabor[]) => void;
}

const LaborTable: React.FC<LaborTableProps> = ({ laborEntries, onUpdate }) => {
  const { globalSettings, addBaseLabor } = useAppStore();
  ...
  const handleAddRow = () => {
    const laborId = Object.keys(globalSettings.baseLabor || {})[0] || '';
    const newLabor: TechnologyLabor = {
      laborId,
      usage: 1.0,
    };
    onUpdate([...laborEntries, newLabor]);
  };

  const handleCreateGlobalLabor = () => {
    if (newName.trim() && newPrice >= 0) {
      addBaseLabor(newName.trim(), newPrice);
      setNewName('');
      setNewPrice(0);
  ...
  const handleRemoveRow = (index: number) => {
    const updated = laborEntries.filter((_, i) => i !== index);
    onUpdate(updated);
  };

  const handleChange = (index: number, field: keyof TechnologyLabor, value: any) => {
    const updated = laborEntries.map((l, i) => {
      if (i === index) {
        ...
        <div className="flex items-center gap-2">
          {!isAddingNew ? (
            ...
            <div className="flex items-center gap-2 bg-indigo-50 p-1.5 rounded-lg border border-indigo-100 animate-in fade-in slide-in-from-right-2">
              ...
              />
              <button 
                onClick={handleCreateGlobalLabor}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50/50 border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest min-w-[350px]">Czynność (Robocizna)</th>
              <th className="px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest w-48">Wsp./Zużycie</th>
              <th className="px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest w-40 text-right">Stawka j.</th>
              ...
              <tr key={idx} className="group transition-colors hover:bg-slate-50/50">
                ...
                <td className="px-6 py-4 text-right">
                  <span className="text-sm font-medium text-slate-500 font-mono">
                    {(globalSettings.baseLabor[l.laborId] || 0).toLocaleString('pl-PL', { minimumFractionDigits: 2 })} zł
                  </span>
                </td>
                <td className="px-6 py-4">
                  <button 
        ...
        {laborEntries.length === 0 && (
          <div className="p-8 text-center text-slate-400 bg-slate-50">
             <p className="text-xs font-medium">Lista robocizny z bazy jest pusta.</p>
          </div>
        )}
      </div>
    </div>
